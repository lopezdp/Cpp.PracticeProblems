...
int main()
{
	...
	int num;                   //total number of items per product
	...
	{
		getline(infile, temp, ' '); //gets string of data one by one
		id = atoi(temp.c_str());
		myInventory[pos].setId(id);

		getline(infile, temp, '\n'); //gets string of data one by one
		num = atoi(temp.c_str());
		myInventory[pos].setAmount(num);
	}
	...
	cout << "Item Number" << setw(15) << "QTY" << endl;
	cout << "==========================" << endl;

	for (pos = 0; pos < NUMOFPROD; pos++)
	{
		myInventory[pos].display();
	}
...
void Inventory::setId(int itemNumber)      // This puts item in the private data member 
{
	this->itemNumber = itemNumber;
}

void Inventory::setAmount(int numOfItem)   // This puts num in the private data member
{
	this->numOfItem = numOfItem;
}

void Inventory::display()            // This prints to the screen 
{
	cout << setw(5) << getIDs() << setw(20) << getAmount() << endl;
}

// Getters
int Inventory::getIDs() //This "gets" the itemNumber for each product in Inventory
{
	return itemNumber;
}

int Inventory::getAmount() //This "gets" the numOfItem for each product in Inventory
{
	return numOfItem;
}
